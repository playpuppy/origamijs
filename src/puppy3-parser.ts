...
const grammar = (start: string) => {
  if (peg === null) {
    peg = {};
    peg['Source'] = pSeq3(pOption(pRef(peg,'EOL')),pNode(pMany(pSeq2(pEdge('',pRef(peg,'Statement')),pRef(peg,'EOL'))),'Source',0),pRef(peg,'EOF'));
    ...
    peg['Statement'] = pOre(pRef(peg,'ClassDecl'),pRef(peg,'ImportDecl'),pRef(peg,'FuncDecl'),pRef(peg,'IfStmt'),pRef(peg,'ForStmt'),pRef(peg,'Return'),pRef(peg,'Yield'),pRef(peg,'Break'),pRef(peg,'Continue'),pRef(peg,'VarDecl'),pRef(peg,'Expression'));
    ...
    peg['Return'] = pNode(pSeq2(pChar('return'),pOption(pSeq3(pRef(peg,'S'),pRef(peg,'_'),pEdge('expr',pRef(peg,'Expression'))))),'Return',0);
    ...
    peg['ForStmt'] = pOre2(pNode(pSeq(pSeq2(pChar('for'),pRef(peg,'S')),pRef(peg,'_'),pEdge('each',pRef(peg,'Name')),pSeq2(pChar('in'),pRef(peg,'S')),pRef(peg,'_'),pEdge('list',pRef(peg,'Expression')),pChar(':'),pRef(peg,'_'),pEdge('body',pOre2(pRef(peg,'Block'),pRef(peg,'Statement')))),'ForStmt',0),pNode(pSeq(pSeq2(pChar('while'),pRef(peg,'S')),pRef(peg,'_'),pEdge('cond',pRef(peg,'Expression')),pChar(':'),pRef(peg,'_'),pEdge('body',pOre2(pRef(peg,'Block'),pRef(peg,'Statement')))),'WhileStmt',0));
    peg['VarDecl'] = pOre2(pNode(pSeq(pEdge('left',pRef(peg,'LeftHand')),pRange('=',[]),pRef(peg,'_'),pEdge('right',pRef(peg,'Expression'))),'VarDecl',0),pRef(peg,'SelfAssign'));
    peg['LeftHand'] = pSeq2(pRef(peg,'Name'),pMany(pOre2(pFold('recv',pSeq3(pRange('.',[]),pRef(peg,'_'),pEdge('name',pRef(peg,'Name'))),'GetExpr',0),pFold('recv',pSeq3(pRef(peg,'"["'),pEdge('index',pRef(peg,'Expression')),pRef(peg,'"]"')),'IndexExpr',0))));
    ...
    peg['Expression'] = pSeq2(pRef(peg,'Operator'),pOption(pFold('then',pSeq(pChar('if'),pRef(peg,'_'),pEdge('cond',pRef(peg,'Expression')),pChar('else'),pRef(peg,'_'),pEdge('else',pRef(peg,'Expression'))),'IfExpr',0)));
    ...
    peg['AndExpr'] = pSeq2(pRef(peg,'EqExpr'),pMany(pFold('left',pSeq3(pRef(peg,'AND'),pRef(peg,'_'),pEdge('right',pRef(peg,'EqExpr'))),'And',0)));
    ...
    peg['EQ'] = pOre(pSeq2(pChar('=='),pNot(pChar('='))),pSeq2(pChar('!='),pNot(pChar('='))),pSeq2(pChar('<='),pNot(pChar('='))),pSeq2(pChar('>='),pNot(pChar('='))),pSeq2(pChar('<'),pNot(pChar('<'))),pSeq2(pChar('>'),pNot(pChar('>'))),pSeq2(pChar('in'),pRef(peg,'S')),pSeq2(pChar('not in'),pRef(peg,'S')),pSeq2(pChar('is'),pRef(peg,'S')),pSeq2(pChar('is not'),pRef(peg,'S')));
    ...
    peg['SUM'] = pOre(pChar('+'),pChar('＋'),pChar('-'),pChar('ー'));
    peg['ProdExpr'] = pSeq2(pRef(peg,'UnaryExpr'),pMany(pFold('left',pSeq3(pEdge('name',pNode(pRef(peg,'PROD'),'Name',0)),pRef(peg,'_'),pEdge('right',pRef(peg,'UnaryExpr'))),'Infix',0)));
    peg['PROD'] = pOre(pChar('//'),pChar('**'),pChar('/'),pChar('*'),pChar('%'),pChar('@'),pChar('^'));
    ...
    peg['PRE'] = pOre(pChar('-'),pChar('!'),pChar('+'),pSeq2(pChar('not'),pRef(peg,'S')));
    peg['SuffixExpr'] = pSeq2(pRef(peg,'Primary'),pMany(pOre(pFold('recv',pSeq(pChar('.'),pRef(peg,'_'),pEdge('name',pRef(peg,'Name')),pRef(peg,'"("'),pEdge('params',pRef(peg,'Arguments')),pRef(peg,'")"')),'MethodExpr',0),pFold('recv',pSeq3(pRange('.',[]),pRef(peg,'_'),pEdge('name',pRef(peg,'Name'))),'GetExpr',0),pFold('name',pSeq3(pRef(peg,'"("'),pEdge('params',pRef(peg,'Arguments')),pRef(peg,'")"')),'ApplyExpr',0),pFold('recv',pSeq3(pRef(peg,'"["'),pEdge('index',pRef(peg,'Expression')),pRef(peg,'"]"')),'IndexExpr',0))));
    ...
    peg['GroupExpr'] = pNode(pSeq(pRef(peg,'"("'),pEdge('',pRef(peg,'Expression')),pMany(pSeq2(pRef(peg,'","'),pEdge('',pRef(peg,'Expression')))),pRef(peg,'")"')),'Tuple',0);
    peg['ListExpr'] = pNode(pSeq(pRef(peg,'"["'),pOption(pSeq2(pEdge('',pRef(peg,'Expression')),pMany(pSeq2(pRef(peg,'","'),pEdge('',pRef(peg,'Expression')))))),pMany(pRef(peg,'","')),pRef(peg,'"]"')),'List',0);
    peg['DataExpr'] = pNode(pSeq(pRef(peg,'"{"'),pEdge('',pRef(peg,'KeyValue')),pMany(pSeq2(pMany(pRef(peg,'","')),pEdge('',pRef(peg,'KeyValue')))),pMany(pRef(peg,'","')),pRef(peg,'"}"')),'Data',0);
    ...
    peg['Identifier'] = pSeq2(pNode(pSeq2(pRange('',['AZ','az']),pMany(pRange('_',['AZ','az','09']))),'Name',0),pRef(peg,'_'));
    ...
    peg['LongString'] = pOre2(pSeq(pChar("''"),pNode(pSeq3(pChar("'"),pMany(pOre(pRef(peg,'ESCAPE'),pSeq2(pNot(pRange("\\'",[])),pAny()),pSeq2(pNot(pChar("'''")),pChar("'")))),pChar("'")),'String',0),pChar("''"),pRef(peg,'_')),pSeq(pChar('""'),pNode(pSeq3(pChar('"'),pMany(pOre(pRef(peg,'ESCAPE'),pSeq2(pNot(pRange('\\"',[])),pAny()),pSeq2(pNot(pChar('"""')),pChar('"')))),pChar('"')),'String',0),pChar('""'),pRef(peg,'_')));
    ...
    peg['Number'] = pSeq2(pOre2(pRef(peg,'FloatExpr'),pRef(peg,'IntExpr')),pOption(pFold('',pEdge('',pRef(peg,'Unit')),'Unit',0)));
    peg['Unit'] = pNode(pRef(peg,'UNIT'),'Symbol',0);
    peg['UNIT'] = pOre(pRange('%％度',[]),pChar('px'),pChar('pt'));
    ...
    peg['NullExpr'] = pSeq2(pNode(pOre2(pChar('None'),pSeq2(pOption(pChar('まだ')),pOre(pChar('ない'),pChar('無い'),pChar('なし'),pChar('無し')))),'Null',0),pRef(peg,'_'));
    peg['EOF'] = pNot(pAny());
    peg['NL'] = pOre2(pChar('\n'),pRef(peg,'EOF'));
    peg['S'] = pRange(' \t\r\u3000、，',[]);
...
example('FormatString',"f'{a}+{1}'")
example('Expression',"f'{a}+{1}'")
example('Number','33.3%')
example('Expression','33.3%')
